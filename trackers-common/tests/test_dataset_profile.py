...
from trackers_common.dataset_profile import load_dataset_profile
...
def _load_detectors_common_loader():
    """Load detectors_common.dataset_profile directly from its source file,
    ...
    spec = importlib.util.spec_from_file_location("detectors_common_dataset_profile", module_path)
...
class TestCrossPackageAgreement:
    """Regression guard: trackers_common's and detectors_common's independently
    duplicated dataset_profile loaders must parse the same file identically."""

    def test_trackers_common_and_detectors_common_loaders_agree(self, tmp_path):
        path = _write_profile(
            tmp_path,
            "size_px: 5.0\nspacing_px: 10.8658\ndescription: shared profile\nextra: 1\n",
        )
        detectors_load_dataset_profile = _load_detectors_common_loader()

        trackers_result = load_dataset_profile(path)
        detectors_result = detectors_load_dataset_profile(path)

        assert trackers_result == detectors_result

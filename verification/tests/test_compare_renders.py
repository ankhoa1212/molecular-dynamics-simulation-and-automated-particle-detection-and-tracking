...
class TestBrightfieldStrategyChoice:
    def test_brightfield_strategy_runs_without_deeptrack(self, cr_module, tmp_path, monkeypatch):
        """render_strategy: brightfield warns and skips rather than crashing
        the whole script when deeptrack isn't installed (it's the only
        strategy in _DEEPTRACK_BACKED_STRATEGIES). Passing 'brightfield' as
        a --strategies value without an argparse error also proves it's a
        valid CLI choice."""
        monkeypatch.setattr(
            cr_module,
            "_load_first_frame",

...
- **LodeSTAR's `nms_distance` must be tuned to this dataset's own particle spacing, not copied from `particle-tracking/lodestar_config.yaml`'s real-data value (30px).** At this repo's default synthetic trajectory (median nearest-neighbor spacing ~10.9px), 30px suppressed almost every true detection down to one per local cluster - confirmed directly that raw pre-NMS detection count was essentially unaffected by `nms_distance`, but recall collapsed from ~0.51 to ~0.12 on the *same* raw detections purely from over-aggressive NMS. `verification/config.yaml`'s `benchmark.lodestar.nms_distance: 5` (~1x `psf_sigma`) is the corrected, dataset-appropriate value.
